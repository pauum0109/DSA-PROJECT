...
Purpose: Defines multiple constructors to represent different statuses and associated data in a Go game.
*/
package control;
...
public class Object implements Externalizable, Constants {
    public int status = -1;
    ...
    public Object(int status) {
        this.status = status;  //PING  
    }

    public Object(int status, int playPart, int handiNO, boolean isSelected) {
    ...
    public Object(int status, Point playPoint) {
    ...
    public Object(int status, String talkString) {
        this.status     = status;  //TALKING and ASKING and CONFIRM
        this.talkString = talkString;
    }

    public Object(int status, boolean isAgree, String answerWhat) {
        this.status = status;   //ANSWER
        this.isAgree = isAgree;  // false: disagree; true: agree.
        this.answerWhat = answerWhat;
    }

    public Object(int status, Point playPoint, boolean isDead) {
        this.status = status;   //MARKING
        this.playPoint = playPoint;
        this.isDead = isDead;
    }

    public Object(int status, int blockNumber, int blockNO, byte[] speakingBytes) {
