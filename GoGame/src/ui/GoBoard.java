...
Purpose: Implementation of a Go board
*/
package ui;
...
public class GoBoard extends JPanel implements ActionListener {
    ...
    private Vector<Button> buttonVect;   //Save history and Status.
    private Button[][] playButtons;  //Just Save Current Status.
    ...
    public GoBoard() {
        ...
        buttonVect  = new Vector<Button>(100, 10);
        playButtons = new Button[19][19];        
        ...
        blackButImage = ImageManager.load("black.png", this);
        whiteButImage = ImageManager.load("white.png", this);
    }
    ...
    private void playHandiButton(int x, int y) {
        ...
        Button tempButton = new Button(x, y, goButtonColor);

        tempButton.neighbors = new Button[4];
    ...
    public void indicateCurrentPosition(Point pressPoint, int whoseTurn) {
        ...
        if (playButtons[x][y] == null) {
            ...
            Button tempButton = createTempButton(x, y, goButtonColor);            
    ...
    public boolean oneStepPlay(Point pressPoint, int whoseTurn) {
        ...
        if (playButtons[x][y] == null) {
            ...
            if (tempButton.canBePlayed()) {
                ...
                isMyTurn = !isMyTurn;
                isChanged = true;
                Menu.setMIStatus(MenuConstants.BACKGOSTR, true);
                Menu.setMIStatus(MenuConstants.SAVESTR, true);
                Menu.setMIStatus(MenuConstants.SAVEASSTR, true);
                ...
                currentButton = tempButton;
                buttonVect.addElement(tempButton);
                if (buttonVect.size() > 20) Menu.setMIStatus(MenuConstants.COUNTSTR, true);
                playButtons[x][y] = tempButton;
                currentPosition = null;  //do not indicate position at this time.
                setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
                for (int i = 0; i < 4; i++) {
                    if (tempButton.neighbors[i] != null) {
                        Button neighborB = tempButton.neighbors[i]; 
    ...
    private Button createTempButton(int x, int y, Color goButtonColor) {
        Button tempButton = new Button(x, y, goButtonColor);
    ...
    public int oneStepBack() {
        Button lastButton = (Button)buttonVect.lastElement();
        ...
        Menu.setMIStatus(MenuConstants.SAVESTR, true);
        Menu.setMIStatus(MenuConstants.SAVEASSTR, true);
        if (buttonVect.size() <= 20) Menu.setMIStatus(MenuConstants.COUNTSTR, false);
        
        for (int i = 0; i < 4; i++) {
            if (lastButton.neighbors[i] != null) {
                Button tempButton = lastButton.neighbors[i];
                for (int j = 0; j < 4; j++) {
        ...
        if (lastButton.isCauseButton) {
            for (int i = 0; i < buttonVect.size(); i++) {
                Button tempButton = (Button)buttonVect.elementAt(i);
        ...
        if (!buttonVect.isEmpty()) {
            ...
            currentButton = (Button)buttonVect.lastElement();  //The last element shouldn't be in isDeleted status.
    ...
    private void markSelfAndNeighbors(Button seedButton, boolean isDead) {
    ...
    public PointsAlive countPointsAlive() {
        ...
        return new PointsAlive(blackPoints, whitePoints);
    }
